...
def incremental_phase_3(args, tg_model, ref_model, tg_optimizer, tg_lr_scheduler, \
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, (inputs, targets) in enumerate(trainloader):
            ...
            if is_start_iteration:
                ...
                # baseline,蒸馏全部的特征
                # loss3 = nn.CosineEmbeddingLoss()(cur_features, ref_features.detach(),
                #                                  torch.ones(cur_features.shape[0]).to(device)) * lamda

                # loss3 = nn.CosineEmbeddingLoss()(cur_features[old_index], ref_features[old_index].detach(),
                #                                  torch.ones(cur_features[old_index].shape[0]).to(device)) * lamda

                # loss31 = ASCkd(cur_features_1, ref_features_1, 1, device) * 3
                # loss32 = ASCkd(cur_features_2, ref_features_2, 2, device) * 2
                # loss33 = ASCkd(cur_features_3, ref_features_3, 3, device) * 1

                # loss31 = ASCkd(cur_features_1[old_index], ref_features_1[old_index], 1, device) * 3
                # loss32 = ASCkd(cur_features_2[old_index], ref_features_2[old_index], 2, device) * 2
                # loss33 = ASCkd(cur_features_3[old_index], ref_features_3[old_index], 3, device) * 1

                # loss31 = SCkd(cur_features_1, ref_features_1, device) * 3
                # loss32 = SCkd(cur_features_2, ref_features_2, device) * 2
                # loss33 = SCkd(cur_features_3, ref_features_3, device) * 1

                loss31 = SCkd(cur_features_1[old_index], ref_features_1[old_index], device) * 3
                loss32 = SCkd(cur_features_2[old_index], ref_features_2[old_index], device) * 2
                loss33 = SCkd(cur_features_3[old_index], ref_features_3[old_index], device) * 1
