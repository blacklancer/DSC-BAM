...
def Skd(a, b, device=None, normalize=True):
    ...
    loss_c = nn.KLDivLoss()(F.log_softmax(similarity_ac / T, dim=1), F.softmax(similarity_bc.detach() / T, dim=1)) * a_c.shape[0]
    ...
    loss_s = nn.KLDivLoss()(F.log_softmax(similarity_as / T, dim=1), F.softmax(similarity_bs.detach() / T, dim=1)) * a_s.shape[0]
...
def incremental_phase_7(args, tg_model, ref_model, tg_optimizer, tg_lr_scheduler, \
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, (inputs, targets) in enumerate(trainloader):
            ...
            if is_start_iteration:
                ...
                loss3 = (loss31 * 3 + loss32 * 2 + loss33 * 1) * lamda * 1.2
                loss4 = (loss41 * 3 + loss42 * 2 + loss43 * 1) * lamda * 1
                loss5 = (loss51 * 3 + loss52 * 2 + loss53 * 1) * lamda * 0.5

            loss = loss1 + loss2 + loss3*1 + loss4*1 + loss5*1
            loss.backward()
            tg_optimizer.step()
