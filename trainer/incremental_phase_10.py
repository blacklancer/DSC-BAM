...
def Tkd(cur, ref, tg_model_b, ref_model_b, device=None):
...
def SCkd(a, b, device=None, normalize=True):
    ...
    if normalize:
        a_h = F.normalize(a_h, dim=1, p=2)
        b_h = F.normalize(b_h, dim=1, p=2)
    a_h = a_h.to(device)
    b_h = b_h.to(device)
    loss1 = nn.CosineEmbeddingLoss()(a_h, b_h.detach(), torch.ones(a_h.shape[0]).to(device))
    a_w = a.sum(dim=2).view(a.shape[0], -1)# shape of (b, c * h)
    b_w = b.sum(dim=2).view(b.shape[0], -1)
    if normalize:
        a_w = F.normalize(a_w, dim=1, p=2)
        b_w = F.normalize(b_w, dim=1, p=2)
    a_w = a_w.to(device)
    b_w = b_w.to(device)
    loss2 = nn.CosineEmbeddingLoss()(a_w, b_w.detach(), torch.ones(a_w.shape[0]).to(device))
    return loss1 + loss2
...
#     a = a.to(device)
#     b = b.to(device)
#     sim_a = F.cosine_similarity(a.unsqueeze(1), a.unsqueeze(0), dim=2)
#     sim_a = torch.ones_like(sim_a) - sim_a
#     sim_b = F.cosine_similarity(b.unsqueeze(1), b.unsqueeze(0), dim=2)
#     sim_b = torch.ones_like(sim_b) - sim_b
#     loss = nn.KLDivLoss()(F.log_softmax(sim_a / T, dim=1), F.softmax(sim_b.detach() / T, dim=1)) * a.shape[0]
#     return loss



def incremental_phase_10(args, tg_model, ref_model, tg_optimizer, tg_lr_scheduler, trainloader, testloader,
    ...
    for epoch in range(args.epochs):
        ...
        for batch_idx, (inputs, targets) in enumerate(trainloader):
            ...
            if is_start_iteration:
                ...
                loss31, loss41, loss51 = Tkd(cur_features_1[old_index], ref_features_1[old_index],
                                               tg_model.bam1, ref_model.bam1, device)
                loss32, loss42, loss52 = Tkd(cur_features_2[old_index], ref_features_2[old_index],
                                               tg_model.bam2, ref_model.bam2, device)
                loss33, loss43, loss53 = Tkd(cur_features_3[old_index], ref_features_3[old_index],
                                               tg_model.bam3, ref_model.bam3, device)

                loss3 = (loss31 * 3 + loss32 * 2 + loss33 * 1) * lamda * 1
                loss4 = (loss41 * 3 + loss42 * 2 + loss43 * 1) * lamda * 0.5
                loss5 = (loss51 * 3 + loss52 * 2 + loss53 * 1) * lamda * 1
