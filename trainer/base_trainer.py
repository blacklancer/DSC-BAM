...
import models.resnet_cifar_dsc as resnet_cifar_dsc
import models.resnet_cifar_dbam as resnet_cifar_dbam
...
class BaseTrainer(object):
    ...
    def set_save_path(self):
        ...
        self.save_path = self.log_dir + self.args.dataset + \
                         ...
                         '_np' + str(self.args.nb_protos) + \
                         '_seed' + str(self.args.random_seed) + \
                         '_epoch_a_' + str(self.args.epochs)
    ...
    def set_dataset_variables(self):
        ...
        if self.args.dataset == 'cifar100':
            ...
            self.trainAset = torchvision.datasets.CIFAR100(root='./data',
                                                          train=True,
                                                          download=False,
            ...
            # self.network = resnet_cifar_dsc.resnet32
            self.network = resnet_cifar_dbam.resnet32
    ...
    def imprint_weights(self, tg_model, iteration, X_train, map_Y_train, dictionary_size):
        ...
        if self.args.dataset == 'cifar100':
            ...
            tg_feature_model = nn.Sequential(tg_model.conv1, tg_model.bn1, tg_model.relu,
                                             tg_model.layer1, tg_model.bam1, tg_model.layer2, tg_model.bam2,
                                             tg_model.layer3, tg_model.bam3,
                                             tg_model.avgpool, tg_model.flatten)
    ...
    def update_train_and_valid_loader(self, X_train, map_Y_train, X_valid_cumul, map_Y_valid_cumul):
        ...
        if self.args.dataset == 'cifar100':
            ...
            #                                           shuffle=True,
            #                                           num_workers=self.args.num_workers)
            self.trainAset.data = X_train.astype('uint8')
            self.trainAset.targets = map_Y_train
            trainloader = torch.utils.data.DataLoader(self.trainAset,
                                                      batch_size=self.args.train_batch_size,
                                                      shuffle=True,
                                                      num_workers=self.args.num_workers)
